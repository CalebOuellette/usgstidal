...
export default async function buildGraph(dataObs, dataPred)
{
...
function doCallback(data, hasPOdata)
{
    ...
    else if (hasPOdata === true)
    {
        ...
        for (i = spot + 1; i < data.length; i++)
        {
            ...
            data[i][2] === undefined
                ? gusts.push(null)
                : gusts.push(parseFloat(Math.round((parseFloat(Math.round(data[i][2] * 100) / 100).toFixed(2)) * scale) * 100) / 100).toFixed(2);
